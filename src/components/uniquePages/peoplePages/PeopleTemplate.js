...
const PeopleTemplate = ({ match }) => {
  const res = useFetch(`https://api.salemstate.edu/people/${match.params.id}`);
    ...
    <PeoplePageWrapper>
      <PeopleSection>
        <MainInfo>
          ...
          {res.isLoading ? null : res.response.Profile_Picture ===
            null ? null : (
            <ProfilePicture
              src={`https://api.salemstate.edu${res.response.Profile_Picture.url}`}
              alt={`${res.response.Last_Name}'s Profile Picture`}
            />
          )}

          {res.isLoading ||
          res.response.RoomNumb === null ||
          res.response.RoomNumb === "" ? null : (
            <SecondaryText>
          ...
          {res.isLoading ||
          res.response.Phone__Number === null ||
          res.response.Phone__Number === "" ? null : (
